...
OPEN TTableNames
FETCH NEXT
FROM TTableNames INTO @fTableName
...
DECLARE FK_FIELDS_CUSROR CURSOR FOR SELECT
	FKCOLUMN_NAME,
	PKCOLUMN_NAME,
	UPDATE_RULE,
	DELETE_RULE
...
SET FKType =
			CASE
				WHEN @fUpdateRule = 0 THEN FKType + ' ON UPDATE CASCADE'
				ELSE FKType
			END
WHERE FKName = @FKName
UPDATE #Temp1
SET FKType =
			CASE
				WHEN @fDeleteRule = 0 THEN FKType + ' ON DELETE CASCADE'
				ELSE FKType
			END
WHERE FKName = @FKName
...
SELECT
	SCRIPT = 'ALTER TABLE [dbo].[' + FTName + '] ADD
