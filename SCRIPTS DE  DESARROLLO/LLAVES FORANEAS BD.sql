	...
	WHERE system_type_id = 56 AND type = 'U' AND NOT column_id = 1 AND ( ( NOT @TableName = '' AND Objeto.name = @TableName ) OR @TableName = '') AND ( Objeto.name + Columna.name ) NOT IN ( SELECT Tabla COLLATE DATABASE_DEFAULT + Columna COLLATE DATABASE_DEFAULT FROM #LlavesForaneas );
	...
	SELECT #CamposTabla.Tabla,
		   ...
		   'IF NOT EXISTS(SELECT 1 FROM SYS.FOREIGN_KEYS WHERE PARENT_OBJECT_ID = OBJECT_ID(''' + @TableName + ''')AND name = ''FK_' + #CamposTabla.Tabla + '_'+ #CamposTabla.Columna + ''') 
		   ALTER TABLE [dbo].['+ #CamposTabla.Tabla + '] ADD CONSTRAINT FK_' + #CamposTabla.Tabla + '_' + #CamposTabla.Columna + '' + 
																CASE WHEN NOT #LlavesPrimarias.Columna = #CamposTabla.Columna THEN
																			REPLACE (#CamposTabla.Columna, #LlavesPrimarias.Columna, '')
																		ELSE
																			''
																	END + ' ' + 'FOREIGN KEY(' + #CamposTabla.Columna + ') REFERENCES [dbo].['+ #LlavesPrimarias.Tabla + '] (' + #LlavesPrimarias.Columna + ')' + CHAR (10) + 'GO',
		   ISNULL ( ( SELECT 'SI'
