...
SELECT *
--	UPDATE SalarioAnterior SET SalarioAnterior.FechaFin = '2024-12-31'
...
SELECT SalarioAnterior.IdEmpresaNomina, empleado.IdEmpleado, FechaInicio = '2025-01-01', FechaFin = '1900-01-01', Salario.sd, BaseCotizar = ROUND(ISNULL(Salario.sdi, 0), 2), SDI = ROUND(ISNULL(Salario.sdi, 0), 2)
FROM dbo.tNOMempleadosSalarios SalarioAnterior
INNER JOIN dbo.vPERempleados empleado ON empleado.IdEmpleado = SalarioAnterior.IdEmpleado
INNER JOIN ##Salarios Salario ON empleado.Codigo COLLATE DATABASE_DEFAULT = Salario.Codigo 
WHERE SalarioAnterior.FechaFin = '2024-12-31' AND SalarioAnterior.IdEmpresaNomina = 8
