...
DECLARE FacturaGlobal CURSOR FAST_FORWARD READ_ONLY LOCAL FOR

SELECT IdPeriodo
FROM dbo.tCTLperiodos
WHERE IdPeriodo BETWEEN dbo.fGETidPeriodo ('20181230') AND dbo.fGETidPeriodo ('20201030');

OPEN FacturaGlobal;

FETCH NEXT FROM FacturaGlobal
INTO @IdPeriodo;
...
BEGIN
    ...
    SELECT Operacion.IdOperacion,
        ...
        IdImpuesto = CASE WHEN AsientoD.Campo IN ('InteresMoratorioPagado', 'InteresMoratorioPagadoVencido', 'InteresOrdinarioPagado', 'InteresOrdinarioPagadoVencido') THEN ImpuestoTransaccionFinanciera.IdImpuesto
                          ELSE ImpuestoOperacionD.IdImpuesto
                     END,
        InteresOrdinario = CASE WHEN AsientoD.Campo IN ('InteresOrdinarioPagado', 'InteresOrdinarioPagado') THEN ISNULL (TransaccionFinanciera.InteresOrdinarioPagado, 0) + ISNULL (TransaccionFinanciera.InteresOrdinarioPagadoVencido, 0)
                                ELSE 0
                           END,
        IVAInteresOrdinario = CASE WHEN AsientoD.Campo IN ('InteresOrdinarioPagado', 'InteresOrdinarioPagadoVencido') THEN TransaccionFinanciera.IVAInteresOrdinarioPagado
                                   ELSE 0
                              END,
        InteresMoratorio = CASE WHEN AsientoD.Campo IN ('InteresMoratorioPagado', 'InteresMoratorioPagadoVencido') THEN ISNULL (TransaccionFinanciera.InteresMoratorioPagado, 0) + ISNULL (TransaccionFinanciera.InteresMoratorioPagadoVencido, 0)
                                ELSE 0
                           END,
        IVAInteresMoratorio = CASE WHEN AsientoD.Campo IN ('InteresMoratorioPagado', 'InteresMoratorioPagadoVencido') THEN TransaccionFinanciera.IVAInteresMoratorioPagado
                                   ELSE 0
                              END,
        IdBienServicio = CASE WHEN AsientoD.Campo IN ('InteresMoratorioPagado', 'InteresMoratorioPagadoVencido') THEN -2020
                              WHEN AsientoD.Campo IN ('InteresOrdinarioPagado', 'InteresOrdinarioPagadoVencido') THEN -2019
